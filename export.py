...
def export_detector_homoAdapt_gpu(config, output_dir, args):
    ...
    for i, sample in tqdm(enumerate(test_loader)):
        ...
        anns_mask = sample["anns_mask"][0,0,:,:].numpy()
        img_test=img[0,0,:,:].numpy().copy()
        '''
        plt.subplot(1,3,1)
        plt.imshow(anns_mask)

        kernel = np.ones((5, 5), np.uint8)
        anns_mask=cv2.dilate(anns_mask,kernel,iterations = 1)
        ...
        for j in range(pts.shape[1]):
            x,y,prob = pts[:,j]
            if anns_mask[int(y),int(x)]==0:
                filtered_pts.append(pts[:,j])
                #cv2.circle(img_test,(int(x),int(y)), 3, (0,0,255), -1)
        #plt.imshow(img_test)
        #plt.show()
        filtered_pts = np.array(filtered_pts).T
        pts = filtered_pts
        ## top K points
        pts = pts.transpose()
        print("total points: ", pts.shape)
